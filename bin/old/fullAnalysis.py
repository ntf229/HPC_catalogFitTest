...
# This code is limited to the mcmc samples with the highest posterior probability
...
plotPath = '/scratch/ntf229/catalogFitTest/ProspectorFullAnalysisPlots/'
dataPath = '/scratch/ntf229/catalogFitTest/ProspectorFullAnalysisData/'
...
params = ['Mass', 'SFR', 'Attenuated Energy', 'Emitted Energy', 'FUV', 'NUV']
noDustParams = ['Mass', 'SFR', 'FUV', 'NUV']
    ...
    for d in range(len(dustModels)):
        for s in range(len(dataSelection)):
            ...
            for g in range(len(galaxies)):
                ...
                if args.sampling == 'mcmc':
                    fitPath += 'mcmc/dust/'+SFHs[h]+'/'+dustModels[d]+'/'+dataSelection[s]+
                               '/numWalkers'+args.nwalkers+'/numIter'+args.niter+'/'+galaxy+'/'
                    noDustFitPath += 'mcmc/noDust/'+SFHs[h]+'/'+dataSelection[s]+'/numWalkers'+
                                     args.nwalkers+'/numIter'+args.niter+'/'+galaxy+'/'
                elif args.sampling == 'dynesty':
